...
1. When `enable = 0`, no assignment is made to `y`
...
always @(*) begin
    case (sel)
        ...
        2'b01: y = b;
        2'b10: y = c;
        default: y = d;  // Covers all remaining cases
    endcase
end
```
...
**Problem:** In the first branch, `flag` is not assigned, even if the default is present a latch is inferred for `flag`.
...
always @(*) begin
    case (opcode)
        2'b00: begin
            result = a + b;
            flag = 1'b0;        // Explicitly assign all outputs
        end
        2'b01: begin
            result = a - b;
            flag = overflow;
...
```
**Synthesis:** The synthesis output for a MUX using for loop
